...
async function startSupabaseRealtime() {
  if (supabaseRealtime.channel) return;
  if (!window.supabase?.createClient) return;
  if (!supabaseRealtime.client) {
    const config = await fetchSupabaseConfig();
    if (!config) return;
    supabaseRealtime.enabled = true;
    supabaseRealtime.client = window.supabase.createClient(config.url, config.anonKey);
  }
...
function stopSupabaseRealtime() {
  if (supabaseRealtime.client && supabaseRealtime.channel) {
    supabaseRealtime.client.removeChannel(supabaseRealtime.channel);
  }
  supabaseRealtime.channel = null;
}

function startMasterLedgerSync() {
  if (supabaseRealtime.enabled) return;
...
async function initialize() {
  ...
  try {
    ...
    const realtimeConfig = await fetchSupabaseConfig();
    supabaseRealtime.enabled = Boolean(realtimeConfig);
    if (realtimeConfig && window.supabase?.createClient) {
      supabaseRealtime.client = window.supabase.createClient(realtimeConfig.url, realtimeConfig.anonKey);
    }
